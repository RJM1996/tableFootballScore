...
const isTool = computed(() => {
  return currentPath.value.includes('tool')
})
</script>
      ...
      <div>
        <div class="season">
          ...
          <el-select v-model="season" style="width: 100px" @change="onSeasonChange">
            <el-option v-for="item in seasonOpts" :key="item.value" :label="item.label" :value="item.value" />
          </el-select>
        </div>
        <Rank :rank-list="gameData.rankList" :loading="loading"></Rank>
        <ScoreList :loading="loading" :score-list="gameData.scoreList" :season="season" :isAdmin="isAdmin"
          @onSuccess="getRankData"></ScoreList>
      </div>
    </div>
  </div>
</template>
